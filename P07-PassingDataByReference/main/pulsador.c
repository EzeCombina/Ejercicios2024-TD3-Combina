...
void inicializarLed(void)
{
    ...
    {
        led[i].indice              = i;
        led[i].diferenciaTiempo    = TIEMPO_NO_VALIDO;
    }
}
...
void inicializarPulsador(void)
{
    for(int i = 0; i < N_PULSADOR; i++)
    {
        ...
        pulsador[i].cola               = xQueueCreate(1, sizeof(ledInfo*));    // Creación de la cola;  
...
static void botonLiberado(uint8_t  indice)
{
    ...
    led[indice].diferenciaTiempo = pulsador[indice].tiempoAlto - pulsador[indice].tiempoBajo;   // Da el tiempo que el pulsador estuvo en estado alto
    ptr = &led[indice];     // Se le da la definiciión aca para que la referencia sea al indice deseado y no siempre al primero. 
    xQueueSend(pulsador[indice].cola, &ptr, portMAX_DELAY);
}
